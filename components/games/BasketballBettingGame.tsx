...
const BasketballBettingGame: React.FC = () => {
  ...
  const loadOddsForMatch = async (match: BasketballMatch) => {
    ...
    try {
      ...
      // Try different basketball sport categories to find the match
      const basketballSports = ['basketball_nba', 'basketball_ncaab', 'basketball_euroleague'];
  ...
  return (
    <ScrollView style={styles.container}>
          ...
          {odds.map((market) => (
            <View key={market.key} style={styles.marketContainer}>
              <Text style={styles.marketTitle}>
                {market.key === 'match_result' && '🏆 Game Winner'}
                {market.key === 'total_points' && '🏀 Total Points'}
                {market.key === 'point_spread' && '📊 Point Spread'}
                {market.key === 'quarter_result' && '⏰ 1st Quarter'}
              </Text>
      ...
      {gamePhase === 'history' && (
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>📊 My Basketball Bets</Text>
          {bets.length === 0 ? (
...
export default BasketballBettingGame;
