// Mega Draw Lottery Game for Adola App
...
export default function MegaDrawGame() {
  const { user } = useApp();
  ...
  const calculatePayout = (matches: number, betAmount: number) => {
    ...
    return multiplier > 0 ? Math.min(betAmount * multiplier, 1000000) : 0; // Cap at 1M
  };
  ...
  const playLottery = async (betAmount: number) => {
    ...
    try {
      ...
      if (!gameLogicService.canPlayGame(betAmount, balance || 0, 'megadraw')) {
        const message = gameLogicService.getBalanceValidationMessage(betAmount, balance || 0, 'megadraw');
      ...
      const { probability, engagementBonus: bonus } = await gameLogicService.calculateWinProbability({
        betAmount,
        basePayout: 1000, // Max jackpot multiplier
        gameType: 'megadraw',
        userId: user.id,
      ...
      console.log(`🎯 Mega Draw: Win probability ${(probability * 100).toFixed(1)}%, Numbers: [${selectedNumbers.join(', ')}]`);
      if (bonus) {
        console.log(`🎯 Engagement bonus: ${bonus}`);
      }

      // Step 1: Deduct bet amount immediately
      console.log(`Placing Mega Draw bet: PKR ${betAmount} with numbers [${selectedNumbers.join(', ')}]`);
      const betPlaced = await placeBet(betAmount, 'megadraw', `Mega Draw bet - ${selectedNumbers.join(', ')}`);
      if (!betPlaced) {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(`Mega Draw: Win probability: ${(probability * 100).toFixed(1)}%, Player numbers: [${selectedNumbers.join(', ')}], Winning numbers: [${winningNumbers.join(', ')}], Won: ${gameResult.won}`);
          ...
          await gameLogicService.logGameResult(user.id, 'megadraw', {
            ...gameResult,
            won: isWin,
          ...
          if (isWin && finalWinAmount > 0) {
            console.log(`Adding Mega Draw winnings: PKR ${finalWinAmount} for ${matchCount} matches`);
            const winningsAdded = await addWinnings(
              finalWinAmount,
              'megadraw',
              `Mega Draw win - ${matchCount} matches`
            );
          ...
          let prizeLevel = '';
          switch (matchCount) {
            case 6: prizeLevel = 'JACKPOT!'; break;
            case 5: prizeLevel = '2nd Prize!'; break;
            case 4: prizeLevel = '3rd Prize!'; break;
            case 3: prizeLevel = '4th Prize!'; break;
            case 2: prizeLevel = '5th Prize!'; break;
            default: prizeLevel = 'No Prize'; break;
          }

          let message = `${prizeLevel}\nMatches: ${matchCount}/6\nYour numbers: ${selectedNumbers.join(', ')}\nWinning numbers: ${winningNumbers.join(', ')}`;
          if (isWin) {
          ...
          Alert.alert('Mega Draw Result', message, [{ text: 'OK' }]);
        } catch (error) {
          console.error('❌ Error in Mega Draw game logic:', error);
          setIsDrawing(false);
          Alert.alert('Error', 'An error occurred while drawing numbers. Please try again.');
        }
      }, 3000);
    } catch (error) {
      console.error('❌ Error in Mega Draw playLottery:', error);
  ...
  const getPrizeTable = () => {
    return [
      { matches: 6, prize: 'PKR 1,000,000', odds: '1 in 13,983,816' },
      { matches: 5, prize: 'PKR 10,000', odds: '1 in 54,201' },
      { matches: 4, prize: 'PKR 500', odds: '1 in 1,032' },
      { matches: 3, prize: 'PKR 50', odds: '1 in 57' },
      { matches: 2, prize: 'PKR 5', odds: '1 in 8' },
    ];
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>Pick 6 numbers and win the jackpot!</Text>
      ...
      <View style={styles.numberGridContainer}>
        <Text style={styles.sectionTitle}>Choose Your Numbers:</Text>
      ...
      {isDrawing && (
        <View style={styles.drawingContainer}>
          <Text style={styles.drawingText}>🎲 Drawing numbers...</Text>
          <Text style={styles.drawingSubtext}>Good luck!</Text>
        </View>
      )}
      ...
      {lastResult && !isDrawing && (
        <View style={styles.resultContainer}>
          ...
          <Text style={styles.resultMatches}>
            Matches: {lastResult.matchCount}/6
          </Text>
