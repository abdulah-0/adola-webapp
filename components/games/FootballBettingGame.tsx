...
const FootballBettingGame: React.FC = () => {
  ...
  const loadOddsForMatch = async (match: FootballMatch) => {
    ...
    try {
      ...
      for (const sport of footballSports) {
        try {
          const targetUrl = `https://api.the-odds-api.com/v4/sports/${sport}/events/${match.id}/odds?apiKey=${API_KEY}&markets=h2h,totals,btts&oddsFormat=decimal&dateFormat=iso`;
          const response = await fetch(`https://corsproxy.io/?${encodeURIComponent(targetUrl)}`);
