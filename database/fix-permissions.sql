...
-- Allow only admins to update game configs (simplified check)
...
-- Temporary: Allow all authenticated users to update (for testing)
-- Remove this after confirming admin check works
CREATE POLICY "Temp allow all updates" ON public.game_configs
    FOR UPDATE TO authenticated
    USING (true);
